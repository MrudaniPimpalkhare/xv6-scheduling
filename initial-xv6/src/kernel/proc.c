...
found:
  p->pid = allocpid();
  //printf("allocproc: Allocated process %s (pid: %d)\n", p->name, p->pid);
  p->state = USED;
  p->tickets = 1;
...
int getsyscount(uint64 addr, int num)
{
  struct proc *pp;
  int havekids, count;
  struct proc *p = myproc();
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (;;) {
      ...
      //check if processes need boosting
      if(ticks-boost_timer >=BOOST_TICKS){
        struct proc *b;
        for(b = proc; b<&proc[NPROC];b++)
        {
          acquire(&b->lock);
          if(b->state == RUNNABLE){
            b->priority = 0;
            b->time_slice = TIMESLICE_0;
          }
          release(&b->lock);
        }
        boost_timer = ticks;
      ...
      struct proc* found = 0;
      for(int i=0;i<MAX_PR && !found;i++){
        for(int j=0;j<NPROC && !found;j++){
          int index = (track[i] + j) % NPROC;
          p = &proc[index];
          acquire(&p->lock);
          if(p->state == RUNNABLE && p->priority == i) {
            found = p;
            track[i] = (index + 1) % NPROC;
          } else {
            release(&p->lock);
          }
        }
...
getqueue(int pid)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    acquire(&p->lock);
    if(p->pid == pid){
      int queue = p->priority;  // Assuming 'priority' represents the queue
      release(&p->lock);
      return queue;
    }
    release(&p->lock);
