...
void send_chunk(int sockfd, struct sockaddr_in *client_addr, socklen_t addr_len, Chunk *chunk)
{
    sendto(sockfd, chunk, sizeof(Chunk), 0, (struct sockaddr *)client_addr, addr_len);
    printf("Sent chunk %d: %s\n", ntohl(chunk->sequence_number), chunk->data);
}

void send_ack(int sockfd, struct sockaddr_in *client_addr, socklen_t addr_len, uint32_t sequence_number)
{
    uint32_t ack_seq = htonl(sequence_number); // Convert sequence number to network byte order
    sendto(sockfd, &ack_seq, sizeof(ack_seq), 0, (struct sockaddr *)client_addr, addr_len);
    printf("Sent ACK for chunk %d\n", sequence_number);
}

int receive_ack(int sockfd, struct sockaddr_in *client_addr, socklen_t addr_len, uint32_t expected_seq)
{
    uint32_t ack_seq;
    int recv_len = recvfrom(sockfd, &ack_seq, sizeof(ack_seq), 0, (struct sockaddr *)client_addr, &addr_len);
...
void receive_chunks(int sockfd, struct sockaddr_in *client_addr, socklen_t addr_len)
{
    ...
    if (total_chunks == 0)
    {
        recv_len = recvfrom(sockfd, &total_chunks, sizeof(total_chunks), 0, (struct sockaddr *)client_addr, &addr_len);
        if (recv_len > 0)
        {
    ...
    if (total_chunks > 0)
    {
        while (received_count < total_chunks)
        {
            ...
            recv_len = recvfrom(sockfd, &chunk, sizeof(chunk), 0, (struct sockaddr *)client_addr, &addr_len);
            ...
            if (recv_len > 0)
            {
                ...
                {   
                    ...
                    send_ack(sockfd, client_addr, addr_len, sequence_number);
                }
            }
            else if (recv_len < 0 && errno != EAGAIN && errno != EWOULDBLOCK)
            {
...
void send_chunks(int sockfd, struct sockaddr_in *client_addr, socklen_t addr_len, const char *message)
{
    ...
    sendto(sockfd, &total_chunks_net, sizeof(total_chunks_net), 0, (struct sockaddr *)client_addr, addr_len);
    ...
    while (window_start < total_chunks)
    {
        ...
        while (next_seq_num < total_chunks && next_seq_num < window_start + WINDOW_SIZE)
        {
            ...
            {
                send_chunk(sockfd, client_addr, addr_len, &chunks[next_seq_num]);
        ...
        for (int i = window_start; i < next_seq_num; i++)
        {
            if (!chunk_status[i].is_acked)
            {
                ...
                if (elapsed > TIMEOUT)
                {
                    ...
                    send_chunk(sockfd, client_addr, addr_len, &chunks[i]);
    ...
    while (1)
    {
        ...
        if (FD_ISSET(STDIN_FILENO, &readfds))
        {
            if (fgets(message, M_SIZE, stdin) != NULL)
            {
                ...
                send_chunks(sockfd, &client_addr, addr_len, message);
            }
        }
    }
